...
**Solution:** I used **Grafana's built-in AI agent** to generate all 20 queries. It understood my specific metric names (`alloy_resources_cpu_seconds_total`, `coredns_dns_requests_total`) and suggested appropriate aggregations (`sum`, `rate`, `histogram_quantile`).

### The 20 Dashboard Panels (Generated by Grafana AI)
...
- **METRICS (8 panels):** HTTP request rate, HTTP P95 latency, CPU usage, memory usage, active goroutines, OS threads, controller errors, Hubble lost events
...
**One Distributed Tracing Dashboard with 20 panels:**
- 8 Metrics panels
- 6 Logs panels
- 6 Traces panels
